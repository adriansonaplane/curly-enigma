...
(async () => {
  ...
  // The game is meant to open straight off the filesystem, and Chromium blocks
  // fetch() on file:// URLs — but <script> and Image still work. So the same
  // manifest is emitted as a script that assigns a global, and that is what the
  // runtime actually reads. The .json stays for tooling and diffing.
  // The sheets are embedded as data URIs rather than referenced as files.
  // A PNG loaded from a file:// URL TAINTS any canvas it is drawn onto, and
  // every later getImageData on that canvas throws SecurityError — which would
  // break the game's own pixel work and every test that samples the frame. A
  // data URI is same-origin by definition, so nothing gets tainted. The .png
  // files stay on disk for review and diffing; this is what the game loads.
  const embedded = {
    ...manifest,
    entries: manifest.entries.map(e => ({
      slug: e.slug, sheet: e.sheet,
      data: 'data:image/png;base64,' +
            fs.readFileSync(path.join(OUT, e.sheet)).toString('base64'),
    })),
  };
  fs.writeFileSync(path.join(OUT, 'index.js'),
    '// generated by tools/bake-effects.js — do not edit\n' +
    '// sheets are inlined as data URIs so they never taint a canvas\n' +
    'window.BAKED_EFFECTS = ' + JSON.stringify(embedded) + ';\n');
